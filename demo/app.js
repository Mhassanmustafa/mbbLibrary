...
export default class App extends Component {
  ...
  handleListenAnnotation = (msg) => {
    ...
    if (typeof data.page === "number") {
      if (
        data.type === "add_annotation" ||
        data.type === "add_markup_annotation"
      ) {
        ...
        if (
          Array.isArray(annotation[data.page]) &&
          annotation[data.page].length > 0
        ) {
  ...
  render() {
    return (
      <View style={{ flex: 1, justifyContent: "center" }}>
        <Button title={"launch"} onPress={this.go} />
      </View>
    );
  }
}
